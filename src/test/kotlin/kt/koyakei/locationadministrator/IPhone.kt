...
import kt.koyakei.propertyRegister.domain.PropertyItem
...
class IPhone {
    fun main() {
        ...
        val スマホのタッチパネル = PropertyItem(
            PropertyItem.Identifier(2),
            listOf(),
            PropertyItem.分類された要素(スマホtouchPanelの分類.identifier),
            "スマホのタッチパネル",
...
// このモデルの電池の電圧はいくつなんだろう　電池の電圧で比較したい場合は？
        ...
        val IPhone = PropertyItem(
            PropertyItem.Identifier(2),
            listOf(
                ひとつの製品に含まれるスマホのタッチパネル部品の状態
            ),
            PropertyItem.分類された要素(スマホ分類された要素v.identifier),
            "IPhone SE 1st gen",
            Maker(PersonInLaw.Identifier(1))
        )
    }
