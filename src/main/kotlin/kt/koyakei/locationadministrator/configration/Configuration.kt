package kt.koyakei.locationadministrator.configration
...
class Configuration : DefaultConfigurer() {
    ...
    @Bean
    fun locationPriceEventSourcingRepository(@Qualifier("eventStore") eventStore: EventStore): EventSourcingRepository<LocationPrice?>? {
        return EventSourcingRepository.builder(LocationPrice::class.java).eventStore(eventStore).build()
    }
